...
std::string HelpMessageCli()
{
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Connect to JSON-RPC on <port> (default: %u or testnet: %u)"), 6543, 16543));
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("Arrow RPC client version") + " " + FormatFullVersion() + "\n" + PrivacyInfo();
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  arrow-cli [options] <command> [params]  " + _("Send command to Arrow") + "\n" +
                  "  arrow-cli [options] help                " + _("List commands") + "\n" +
                  "  arrow-cli [options] help <command>      " + _("Get help for a command") + "\n";
